<script lang="ts">
	...
	let {
		...
		invalid = false,
		showInfo = false,
		infoTitle = 'At least 8 characters',
		required = false,
		oninput
	}: Props = $props();
	...
	const inputClass = $derived(
		hasError
			...
			: 'border-gray-200 focus:border-brand dark:border-slate-600'
	);
</script>

<div class="flex w-full flex-col gap-2">
	<div class="flex h-5 items-center justify-between">
		<label for={name} class="font-inter text-[13px] font-medium text-gray-500 dark:text-slate-400">
			{label}
		</label>
		{#if showInfo}
			<span title={infoTitle} class="text-gray-400 dark:text-slate-500">
				<Info size={14} stroke-width={1.75} aria-hidden="true" />
				<span class="sr-only">{infoTitle}</span>
			</span>
		{/if}
	</div>
	<div class="relative">
		<input
			...
			bind:value
			{oninput}
			class="font-inter h-[52px] w-full rounded-[10px] border bg-gray-50 pr-12 pl-4 text-[15px] text-gray-900 outline-none transition-[border-color,box-shadow] placeholder:text-gray-400 focus:ring-2 focus:ring-brand/20 dark:bg-slate-800 dark:text-slate-100 dark:placeholder:text-slate-600 {inputClass}"
		/>
		<button
			type="button"
